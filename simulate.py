...
def call_agent(agent_name: str, prompt: str, day: int, dry_run: bool) -> str:
    ...
    try:
        ...
        log(label, "ERROR: Cannot connect to OpenClaw gateway at " + GATEWAY_URL)
        log(label, "Run: openclaw gateway restart")
        sys.exit(1)
    except requests.exceptions.HTTPError as e:
        log(label, f"HTTP ERROR: {e}")
        log(label, f"Response body: {resp.text[:500]}")
        sys.exit(1)
